"""sem_agent/affordance_graph.py — Self-contained affordance graph builder.
...
def render_affordance_relation_map(graph):
    """Render the graph as a compact LLM navigation view."""
    if not isinstance(graph, dict):
        return "(no affordance relations available)"

    entity_names = [
        str(e["entity"]).strip()
        for e in graph.get("entity_types", [])
        if isinstance(e, dict) and str(e.get("entity", "")).strip()
    ]
    ...
    for source in graph.get("source_affordances", []):
        ...
        connects = " <-> ".join(str(v) for v in source.get("connects", []) if str(v))
        relations = "; ".join(str(v) for v in source.get("recorded_relations", []) if str(v)) \
        ...
        derived = [str(v) for v in source.get("derived_from", []) if str(v)]
        if derived:
            line += " | derived_from=" + ",".join(derived)
    ...
    lines.extend([
        "COMPOSITION: invert typed relations when valid; join through shared entity types; use "
        "representations to retrieve anchors, then continue into observed bundle, user, category, "
...
def build_evidence_affordance_graph(source_manifest, dataset):
    """Build a reusable connectivity map from the available sources."""
    ...
    graph = {
        ...
        "entity_types": [
            {"entity": "item", "identity_rule": "canonical integer item_id",
            ...
            {"entity": "item_representation", "identity_rule": "validated vector row aligned to canonical item_id"},
            {"entity": "dataset_statistics", "identity_rule": "dataset-level aggregate"},
        ],
        "source_affordances": [_source_affordance(s) for s in sources],
