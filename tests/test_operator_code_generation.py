...
def _operator_code_generation_prompt(
    ...
    return (
        ...
        "Every reported value or observation must identify its source file and explain its relation "
        "to the partial bundle or candidate.\n\n"
        "CAPABILITY-TO-FILE BINDINGS\n"
        f"{pretty_json(capability_bindings)}\n\n"
        "AVAILABLE WORKSPACE SOURCES\n"
        f"{pretty_json(source_manifest)}\n\n"
        "Read only listed relative paths under data/. Use only sources required by SELECTED_OPERATORS. "
        "Load .pt files on CPU. Do not access network resources, parent directories, "
